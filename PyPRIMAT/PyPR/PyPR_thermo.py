...
if(PyPRini.numba_flag):
    from numba import njit

my_dir = PyPRini.working_dir
if(PyPRini.verbose_flag):
    print("PyPR_thermo.py: Loading SM rates for thermal bath")
...
fnu_e_scat_tab = np.loadtxt(my_dir+"/PyPRrates/thermo/"+"nue_scatt.txt")
fnu_e_scat = interp1d(fnu_e_scat_tab[:,0],fnu_e_scat_tab[:,1], bounds_error=False, fill_value="extrapolate", kind='linear')
fnu_mu_scat_tab = np.loadtxt(my_dir+"/PyPRrates/thermo/"+"numu_scatt.txt")
...
fnu_e_ann_tab = np.loadtxt(my_dir+"/PyPRrates/thermo/"+"nue_ann.txt")
fnu_e_ann = interp1d(fnu_e_ann_tab[:,0],fnu_e_ann_tab[:,1], bounds_error=False, fill_value="extrapolate", kind='linear')
fnu_mu_ann_tab = np.loadtxt(my_dir+"/PyPRrates/thermo/"+"numu_ann.txt")
...
if(PyPRini.numba_flag):
    @njit
    def rho_e_int(E,Tg):
        return E**2*(E**2-(PyPRini.me/Tg)**2)**0.5/(np.exp(E)+1.)
else:
    def rho_e_int(E,Tg):
        return E**2*(E**2-(PyPRini.me/Tg)**2)**0.5/(np.exp(E)+1.)
def rho_e(Tg):
    if Tg < PyPRini.me/30.:
        ...
        res_int = quad(rho_e_int,PyPRini.me/Tg,100.,args=(Tg),epsabs=1e-12,epsrel=1e-12)[0]
        return 4./(2*np.pi**2)*Tg**4*res_int
# drho_e/dT
if(PyPRini.numba_flag):
    @njit
    def drho_e_dT_int(E,Tg):
        return E**3*(E**2-(PyPRini.me/Tg)**2)**0.5/np.cosh(E/2.0)**2
else:
    def drho_e_dT_int(E,Tg):
        return E**3*(E**2-(PyPRini.me/Tg)**2)**0.5/np.cosh(E/2.0)**2
def drho_e_dT(Tg):
    if Tg < PyPRini.me/30.:
        ...
        res_int = quad(drho_e_dT_int,PyPRini.me/Tg,100.,args=(Tg),epsabs=1e-12,epsrel = 1e-12)[0]
        return 1./(2*np.pi**2)*Tg**3*res_int
# e+- pressure density
if(PyPRini.numba_flag):
    @njit
    def p_e_int(E,Tg):
        return (E**2-(PyPRini.me/Tg)**2)**1.5/(np.exp(E)+1.)
else:
    def p_e_int(E,Tg):
        return (E**2-(PyPRini.me/Tg)**2)**1.5/(np.exp(E)+1.)
def p_e(Tg):
    if Tg < PyPRini.me/30.:
        ...
        res_int = quad(p_e_int,PyPRini.me/Tg,100.,args=(Tg),epsabs=1e-12,epsrel=1e-12)[0]
...
def rho_nu(Tnu):
    ...
    rho_nu_extra = PyPRini.DeltaNeff/3.*rho_nu_SM
...
def delta_rho_nue(Tg,Tnue,Tnumu):
    return PyPRini.MeV_to_secm1*PyPRini.GF**2/np.pi**5*(4.*(PyPRini.geL**2+PyPRini.geR**2)*f_nu_e(Tg,Tnue)+2.*f_g(Tnumu,Tnue))
def delta_rho_numu(Tg,Tnue,Tnumu):
    return PyPRini.MeV_to_secm1*PyPRini.GF**2/np.pi**5*(4.*(PyPRini.gmuL**2+PyPRini.gmuR**2)*f_nu_mu(Tg,Tnue)-f_g(Tnumu,Tnue))
