...
if(PyPRini.compute_nTOp_thermal_flag):
    import vegas
...
def FermiCoulomb(b):
    me = PyPRini.me*PyPRini.MeV # electron mass
    Gamma = np.sqrt(1.-PyPRini.alphaem**2.)-1.
    gamma1 = 1.+Gamma
    gamma2 = 3.+2.*Gamma
    Fn_Compton = PyPRini.hbar*PyPRini.clight/me
    return (1.+Gamma/2.)*4.*((2.*PyPRini.radproton*b)/Fn_Compton)**(2.*Gamma)/(gamma(gamma2)**2)* np.exp((np.pi*PyPRini.alphaem)/b)/((1.-b**2)**Gamma)*np.abs(gamma(gamma1+(PyPRini.alphaem/b)*1j))**2
...
def RadCorrResum(b,y,en):
    ...
    # Particle masses
    me = PyPRini.me*PyPRini.MeV # electron mass
    mn = PyPRini.mn*PyPRini.MeV # neutron mass
    mp = PyPRini.mp*PyPRini.MeV # proton mass
    mA = mA*PyPRini.MeV # nucleon mass
    ...
    return (1.+PyPRini.alphaem/(2.*np.pi)*(Sirlin_fun-3.*np.log(mp/(2*Q))))*(Lndecay+(PyPRini.alphaem/np.pi)*Cndecay+ PyPRini.alphaem/(2*np.pi)*deltandecay*2*np.pi/PyPRini.alphaem)*(Sndecay+1./(134.*2.*np.pi)*(np.log(mp/mA)+Agndecay)+NLLndecay)
...
def ComputeFn():
    ...
    def ChiFMnDec(en,pe):
        mnOme = mn/me
        f1n = ((1.+PyPRini.gA)**2.+2.*PyPRini.deltakappa*PyPRini.gA)/(1.+3.*PyPRini.gA**2)
        f2n = ((1.-PyPRini.gA)**2.-2.*PyPRini.deltakappa*PyPRini.gA)/(1.+3.*PyPRini.gA**2)
        f3n = (PyPRini.gA**2-1.)/(1.+3.*PyPRini.gA**2)
...
def ComputeWeakRates(Tvec):
    ...
    xi_nu = PyPRini.munuOverTnu # neutrino chemical potential over temperature
    my_dir = PyPRini.working_dir
    Tg_vec,Tnu_vec = Tvec # photon and neutrino temperatures
    Tg_Kelvin = Tg_vec*PyPRini.MeV_to_Kelvin
    ...
    def L_nTOpBORN_int(p,T):
        x = me/(PyPRini.kB*T)
        pemax = max(7.,30./x)
        xnu = me/(PyPRini.kB*T*T_nuOverT(T))
    ...
    def ChiFunc_FM(en, pe, x, znu, sgnq):
        ...
        f_1 = ((1.+sgnq*PyPRini.gA)**2.+2.*PyPRini.deltakappa*sgnq*PyPRini.gA)/(1.+3.*PyPRini.gA**2)
        f_2 = ((1.-sgnq*PyPRini.gA)**2.-2.*PyPRini.deltakappa*sgnq*PyPRini.gA)/(1.+3.*PyPRini.gA**2)
        f_3 = (PyPRini.gA**2-1.)/(1.+3.*PyPRini.gA**2)
        FD2_en = FD2(-en,x)
    ...
    if(PyPRini.compute_nTOp_thermal_flag):
        ...
        def IPENCCRT(E, k, x, znu, sgnq):
            ...
            return  PyPRini.alphaem/(2*np.pi)*(BE(x*k)/k)*(A(E, k)*(FD2(-E,x)*FermiStat(sgnq, 1, pE/E)*(Chitilde(E - k, znu, sgnq) + Chitilde(E + k, znu, sgnq) - 2*Chitilde(E, znu, sgnq))+ FD2(E, x)*FermiStat(sgnq, -1, pE/E)*(Chitilde(-E + k, znu, sgnq) + Chitilde(-E - k, znu, sgnq) - 2*Chitilde(-E, znu, sgnq)))-k*B(E)*(FD2(-E, x)*FermiStat(sgnq, 1, pE/E)*(Chitilde(E - k, znu, sgnq) - Chitilde(E + k, znu, sgnq))+ FD2(E, x)*FermiStat(sgnq, -1, pE/E)*(Chitilde(-E + k, znu, sgnq)- Chitilde(-E - k, znu, sgnq))))
        ...
        def IPENCCRDiffBremsstrahlung(E, k, x, znu, sgnq):
            ...
            def Chitilde(en, znuval, sgnq):
                ...
                resvec[my_index[:]] = 1./(np.exp(argvec[my_index[:]]) + 1.)
                return resvec*(en-sgnq*q)**2
            res_fac = PyPRini.alphaem/(2.*np.pi*k)
        ...
        def C1dE(E, x, znu, sgnq):
            pE = np.sqrt(E**2-1.)
            return -((PyPRini.alphaem*E)/(2.*np.pi*pE))*(2.*np.pi**2)/(3.*x**2)*(ChiFunc(E,pE,x,znu,sgnq)+ ChiFunc(-E,pE,x, znu,sgnq))
        ...
        def L_nTOpThermalTruePhoton(T):
            ...
            max_E = max(10.,20./x)
            min_k = 0.001
            max_k = max(10.,20./(me/(PyPRini.kB*T)))
        ...
        def L_nTOpThermalDiffBremsstrahlung(T):
            min_E = 1.001
            max_E = max(10.,20./(me/(PyPRini.kB*T)))
            min_k = 0.001
            max_k = max(10.,20./(me/(PyPRini.kB*T)))
            integ = vegas.Integrator([[min_E,max_E],[min_k,max_k]])
            @vegas.batchintegrand
        ...
        def L_nTOpThermal_1_int(E, T):
            return C1dE(E, me/(PyPRini.kB*T), me/(PyPRini.kB*T*T_nuOverT(T)), 1)

        def L_nTOpThermal_1(T):
            return quad(L_nTOpThermal_1_int, 1., max(25., 150.*(PyPRini.kB*T)/me), args=(T), epsrel = 1.e-2)[0]
        ...
        def L_pTOnThermal_1(T):
            return quad(L_pTOnThermal_1_int, 1., max(25., 150.*(PyPRini.kB*T)/me), args=(T), epsrel = 1.e-2)[0]
        ...
        T_nTOp_thermal_interval = np.logspace(np.log10(PyPRini.T_end),np.log10(PyPRini.T_start),PyPRini.sampling_nTOp_thermal)
        L_nTOpCCRTh_res = L_nTOpCCRTh_vec(T_nTOp_thermal_interval)
        L_pTOnCCRTh_res = L_pTOnCCRTh_vec(T_nTOp_thermal_interval)
        if(PyPRini.save_nTOp_thermal_flag):
            np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_thermal_corrections.txt",np.c_[T_nTOp_thermal_interval,L_nTOpCCRTh_res])
            np.savetxt(my_dir+"/PyPRrates/nTOp/"+"pTOn_thermal_corrections.txt",np.c_[T_nTOp_thermal_interval,L_pTOnCCRTh_res])
        if(PyPRini.verbose_flag):
            print("n <--> p thermal corrections computed")
    else:
        T_nTOp_thermal_interval, L_nTOpCCRTh_res = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_thermal_corrections.txt", unpack = True)
        T_nTOp_thermal_interval, L_pTOnCCRTh_res = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"pTOn_thermal_corrections.txt", unpack = True)
    ################################
    ...
    def nTOp_frwrd_(T):
        ...
        if(PyPRini.nTOpBorn_flag):
            ...
            # total n -> p rate
            rate_nTOp = (L_nTOp_T)
        if(PyPRini.NP_nTOp_flag):
            rate_nTOp += PyPRini.NP_delta_nTOp*L_nTOpBORN(T)
    ...
    def nTOp_bkwrd_(T):
        ...
        if(PyPRini.nTOpBorn_flag):
            ...
            rate_pTOn = (L_pTOn_T)
        if(PyPRini.NP_nTOp_flag):
            rate_pTOn += PyPRini.NP_delta_nTOp*L_pTOnBORN(T)
    ...
    T_interval_HT = np.logspace(np.log10(PyPRini.T_start),np.log10(PyPRini.T_weak),PyPRini.sampling_nTOp)
    if(PyPRini.verbose_flag):
        print("Re-computing n <--> p weak rates @ mid T regime")
    T_interval_MT = np.logspace(np.log10(PyPRini.T_weak),np.log10(PyPRini.T_nucl),PyPRini.sampling_nTOp)
    if(PyPRini.verbose_flag):
        print("Re-computing n <--> p weak rates @ low T regime")
    T_interval_LT = np.logspace(np.log10(PyPRini.T_nucl),np.log10(PyPRini.T_end),PyPRini.sampling_nTOp)
    if(PyPRini.save_nTOp_flag):
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_HT.txt",np.c_[T_interval_HT,nTOp_frwrd_vec(T_interval_HT)])
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_HT.txt",np.c_[T_interval_HT,nTOp_bkwrd_vec(T_interval_HT)])
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_MT.txt",np.c_[T_interval_MT,nTOp_frwrd_vec(T_interval_MT)])
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_MT.txt",np.c_[T_interval_MT,nTOp_bkwrd_vec(T_interval_MT)])
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_LT.txt",np.c_[T_interval_LT,nTOp_frwrd_vec(T_interval_LT)])
        np.savetxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_LT.txt",np.c_[T_interval_LT,nTOp_bkwrd_vec(T_interval_LT)])
