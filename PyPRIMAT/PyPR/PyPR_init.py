...
numba_flag = True # if True, speed up some integrations in PyPR_thermo.py with Numba
# Set flag to True if Numdifftools is installed
numdiff_flag = False # if True, numerical derivative in PyPR_main.py via Numdifftools
debug_flag = False

########################################
# Units for PyPRIMAT nuclear network #
########################################
...
###########################
# PRIMAT compilation for nuclear rates
...
Omegabh2 = 0.022425 # baryon abundance by Planck [A.A. 652 (2021)] (no BBN prior)
...
npdg_T9,npdg_median,npdg_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"npdg.txt",unpack = True)
# dp -> He3g
alpha_dpHe3g,beta_dpHe3g,gamma_dpHe3g = 1.6335e+10,1.5,-63.7491
dpHe3g_T9,dpHe3g_median,dpHe3g_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"dpHe3g.txt",unpack = True)
# dd -> He3n
alpha_ddHe3n,beta_ddHe3n,gamma_ddHe3n = 1.73183e+00,0.,-37.9341
ddHe3n_T9,ddHe3n_median,ddHe3n_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"ddHe3n.txt",unpack = True)
# dd -> tp
alpha_ddtp,beta_ddtp,gamma_ddtp = 1.73492e+00,0.,-46.7971
ddtp_T9,ddtp_median,ddtp_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"ddtp.txt",unpack = True)
# tp -> ag
alpha_tpag,beta_tpag,gamma_tpag = 2.61058e+10,1.5,-229.93
tpag_T9,tpag_median,tpag_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"tpag.txt",unpack = True)
# td -> an
alpha_tdan,beta_tdan,gamma_tdan = 5.5369e+00,0.,-204.1236
tdan_T9,tdan_median,tdan_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"tdan.txt",unpack = True)
# ta -> Li7g
alpha_taLi7g,beta_taLi7g,gamma_taLi7g = 1.1133e+10,1.5,-28.6355
taLi7g_T9,taLi7g_median,taLi7g_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"taLi7g.txt",unpack = True)
# He3n -> tp
alpha_He3ntp,beta_He3ntp,gamma_He3ntp = 1.00178e+00,0.0,-8.8630
He3ntp_T9,He3ntp_median,He3ntp_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"He3ntp.txt",unpack = True)
# He3d -> ap
alpha_He3dap,beta_He3dap,gamma_He3dap = 5.5438e+00,0.0,-212.987
He3dap_T9,He3dap_median,He3dap_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"He3dap.txt",unpack = True)
# He3a -> Be7g
alpha_He3aBe7g,beta_He3aBe7g,gamma_He3aBe7g = 1.11289e+10,1.5,-18.4179
He3aBe7g_T9,He3aBe7g_median,He3aBe7g_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"He3aBe7g.txt",unpack = True)
# Be7n -> Li7p
alpha_Be7nLi7p,beta_Be7nLi7p,gamma_Be7nLi7p = 1.00215,0.,-19.0806
Be7nLi7p_T9,Be7nLi7p_median,Be7nLi7p_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"Be7nLi7p.txt",unpack = True)
# Li7p -> aa
alpha_Li7paa,beta_Li7paa,gamma_Li7paa = 4.6898,0.,-201.295
Li7paa_T9,Li7paa_median,Li7paa_expsigma = np.loadtxt(working_dir+"/PyPRrates/nuclear/"+rates_dir+"Li7paa.txt",unpack = True)
###############################
...
def ReloadKeyRates():
    ...
    rates_dir = "key_primat_rates/"
    dir_key_rates = working_dir+"/PyPRrates/nuclear/"+rates_dir
...
dir_other_rates = working_dir+"/PyPRrates/nuclear/other_nucl_rates/"
# Li7p -> aag
