...
def InterpolateWeakRates():
    ...
    nTOp_frwrd_HT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_HT.txt")
    nTOp_frwrd_HT = interp1d(nTOp_frwrd_HT_tab[:,0],nTOp_frwrd_HT_tab[:,1], bounds_error=False,fill_value="extrapolate",kind='quadratic')
    nTOp_bkwrd_HT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_HT.txt")
    ...
    nTOp_frwrd_MT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_MT.txt")
    nTOp_frwrd_MT = interp1d(nTOp_frwrd_MT_tab[:,0],nTOp_frwrd_MT_tab[:,1], bounds_error=False,fill_value="extrapolate",kind='quadratic')
    nTOp_bkwrd_MT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_MT.txt")
    ...
    nTOp_frwrd_LT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_frwrd_LT.txt")
    nTOp_frwrd_LT = interp1d(nTOp_frwrd_LT_tab[:,0],nTOp_frwrd_LT_tab[:,1], bounds_error=False,fill_value="extrapolate",kind='quadratic')
    nTOp_bkwrd_LT_tab = np.loadtxt(my_dir+"/PyPRrates/nTOp/"+"nTOp_bkwrd_LT.txt")
...
def RecomputeWeakRates(Tvec):
    ...
    if(PyPRini.compute_nTOp_flag):
        if(PyPRini.verbose_flag):
            print(" ")
            print("Computing n <--> p weak rates from scratch.")
            print("Change flags in PyPR_init.py otherwise!")
        import PyPR.PyPR_eval_nTOp as PyPRevalnTOp
        T_interval_HT,nTOp_frwrdvec_HT,nTOp_bkwrdvec_HT,T_interval_MT,nTOp_frwrdvec_MT,nTOp_bkwrdvec_MT,T_interval_LT,nTOp_frwrdvec_LT,nTOp_bkwrdvec_LT = PyPRevalnTOp.ComputeWeakRates(Tvec)
        # HT interval: T_start --> T_weak
